...
export const listItem = async (price, name, description) => {
  try {
    ...
    const listingFee = ethers.utils.parseEther('0.0002');
...
export const editBuyOrderPrice = async (itemId, newPrice) => {
  try {
    const contract = await getContract(true);
    const priceInWei = ethers.utils.parseEther(newPrice.toString());
...
// Create a buy order
export const buyOrder = async (price) => {
  try {
    const contract = await getContract(true);
    const priceInWei = ethers.utils.parseEther(price.toString());
    const buyOrderFee = ethers.utils.parseEther('0.00005');
